//
//  iGaiaRenderOperationOffscreenProcessingHelper.m
//  iGaiaCore
//
...
#import "iGaiaRenderOperationOffscreenProcessingHelper.h"
#import "iGaiaLogger.h"

static ui32 k_RENDER_OPERATION_OFFSCREEN_PROCESSING_MODE = 0;

iGaiaRenderOperationOffscreenProcessingHelper::iGaiaRenderOperationOffscreenProcessingHelper(vec2 _frameSize, iGaiaShader::iGaia_E_Shader _shader,const string& _name)
{
    ...
    m_operatingMaterial->Set_Shader(_shader, k_RENDER_OPERATION_OFFSCREEN_PROCESSING_MODE);
...
iGaiaRenderOperationOffscreenProcessingHelper::~iGaiaRenderOperationOffscreenProcessingHelper(void)
{

}

iGaiaTexture* iGaiaRenderOperationOffscreenProcessingHelper::Get_OperatingTexture(void)
{
    return m_operatingTexture;
}

iGaiaMaterial* iGaiaRenderOperationOffscreenProcessingHelper::Get_OperatingMaterial(void)
{
    return m_operatingMaterial;
}

void iGaiaRenderOperationOffscreenProcessingHelper::Bind(void)
{
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBufferHandle);
    ...
    m_operatingMaterial->Bind(k_RENDER_OPERATION_OFFSCREEN_PROCESSING_MODE);
    m_mesh->Get_VertexBuffer()->Set_OperatingShader(m_operatingMaterial->Get_OperatingShader());
    m_mesh->Bind();
}

void iGaiaRenderOperationOffscreenProcessingHelper::Unbind(void)
{
    m_mesh->Get_VertexBuffer()->Unbind();
    m_mesh->Get_IndexBuffer()->Unbind();
    m_operatingMaterial->Unbind(k_RENDER_OPERATION_OFFSCREEN_PROCESSING_MODE);
}

void iGaiaRenderOperationOffscreenProcessingHelper::Draw(void)
{
